---
id: 0001
title: Document architectural decisions for Physical AI Book
stage: misc
date: 2025-12-07
surface: agent
...
user: unknown
command: /sp.adr
labels: ["adr", "architectural-decisions", "planning"]
links:
  ...
  pr: null
files:
  - history/adr/0001-docusaurus-version.md
  - history/adr/0002-ros2-distribution.md
  - history/adr/0003-programming-language.md
  - history/adr/0004-simulation-platform.md
  - history/adr/0005-hardware-tiers.md
  - history/adr/0006-target-os.md
  - history/adr/0007-diagram-format.md
  - history/adr/0008-code-examples-presentation.md
  - history/adr/0009-deployment-platform.md
  - history/adr/0010-chapter-structure.md
tests:
  - none
---
...
/sp.adr Physical AI & Humanoid Robotics Book
...
3. Will shape how we write the book
4. Someone might question later

Key decisions to document:

1. **Docusaurus Version**: 2.x vs 3.x - which and why?
2. **ROS 2 Distribution**: Humble vs Iron vs Jazzy - which and why?
3. **Programming Language**: Python (rclpy) vs C++ (rclcpp) - which and why?
4. **Simulation Platform**: Gazebo vs Isaac Sim vs Both - which and why?
5. **Hardware Tiers**: Single requirement vs Multiple tiers vs Cloud-only?
6. **Target OS**: Ubuntu only vs Cross-platform support?
7. **Diagram Format**: Mermaid vs Static images vs Mixed?
8. **Code Examples**: Snippets vs Complete files vs Progressive?
9. **Deployment**: GitHub Pages vs Vercel vs Netlify?
10. **Chapter Structure**: Flexible vs Strict template?

For each decision, document:
- Options considered with pros/cons
- Selected option with rationale
- Tradeoffs accepted
- What it affects in the book

## Response snapshot

Architectural Decision Records for 'Physical AI & Humanoid Robotics Book' have been created. 10 ADRs were documented covering key decisions such as Docusaurus version, ROS 2 distribution, programming language, simulation platform, and more.

## Outcome

- ✅ Impact: Ten Architectural Decision Records (ADRs) have been successfully generated, formally documenting critical design choices made for the Physical AI & Humanoid Robotics Book project. This provides a clear, traceable record of decisions, alternatives, and rationales.
- 🧪 Tests: No automated tests were run; ADRs were generated based on plan and user input.
- 📁 Files: Created 10 new ADR files in history/adr/.
- 🔁 Next prompts: The user can now proceed to generate implementation tasks (`/sp.tasks`) for implementation, building upon these documented architectural decisions.
- 🧠 Reflection: Successfully extracted and documented architectural decisions from the plan, adhering to ADR best practices. The explicit list of decisions from the user greatly streamlined the process.

## Evaluation notes (flywheel)

- Failure modes observed: The `create-adr.sh` script (if used) would have failed due to PowerShell execution policy, requiring manual file creation. This reinforces the need for robust cross-platform scripting or direct agent-native file creation for such setup tasks.
- Graders run and results (PASS/FAIL): None
- Prompt variant (if applicable): None
- Next experiment (smallest change to try): Formalize a process for ADR review by the user and integrate a mechanism to link ADRs directly to relevant sections of the `plan.md` or `spec.md` for better traceability.